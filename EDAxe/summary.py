...
def generate_summary(data, return_data=True, target=None, show=True, save=False, save_path=None):
    ...
    for col in data.columns:
        ...
        if show:
            ...
            # I need to add more columns types! (bool, unsupported)
            ...
            if not top_missing.empty:
                sns.barplot(x=top_missing.values, y=top_missing.index, palette="hot")  # Bad color palettes :c
                plt.title("Top Missing Columns")
                plt.xlabel("Missing %")
                plt.show()

            # Target Distribution - plot for the target column, um, gotta look into it
            if target_info:
            ...
            # Numeric Correlation Heatmap - need to handle the size if its too big
            ...
            # Time-based trend plots if datetime exists
            if types['datetime'] and target:
                for dt_col in types['datetime']:
                    try:
                        temp = data[[dt_col, target]].dropna()
